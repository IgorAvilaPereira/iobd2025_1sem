...
select musica.titulo, artista.nome, album.titulo from musica inner join album on (album.id = musica.album_id) inner join artista on (artista.id = album.artista_id);
 ...
 CREATE VIEW musicas_da_anitta AS SELECT musica.titulo, artista.nome_artistico from album inner join musica on (musica.album_id = album.id) inner join artista on (artista.id = album.artista_id) where artista_id = 2;
...
SELECT usuario.nome from usuario INNER JOIN playlist on (usuario.id = playlist.usuario_id);
...
SELECT usuario.id, usuario.nome FROM usuario WHERE id in (select playlist.usuario_id from playlist) order by usuario.id;


select usuario.id, usuario.nome from usuario where id in(SELECT usuario.id from usuario INTERSECT SELECT usuario_id from playlist);

-- off-topic: select usuario.id, usuario.nome, count(usuario.id) from usuario inner join playlist on (usuario.id = playlist.usuario_id) group by usuario.id, usuario.nome having count(*) >= 3;

select coalesce(artista.nome_artistico, artista.nome, 'deu xabum') as nm_artista from artista LEFT JOIn album on (artista.id = album.artista_id) where album.artista_id is null;

-- Retorne a quantidade de álbuns por artista
SELECT artista_id,artista.nome, COUNT(*) from album inner join artista on (artista.id = album.artista_id) GROUP BY artista_id, artista.nome;


SELECT artista_id, coalesce(artista.nome_artistico, artista.nome) as nome_artista, COUNT(musica.id) from album inner join artista on (artista.id = album.artista_id) INNER JOIN musica on (album.id = musica.album_id) GROUP BY artista_id, artista.nome, artista.nome_artistico;

 select musica.titulo from usuario inner join playlist on (usuario.id = playlist.usuario_id) inner join playlist_musica on (playlist.id = playlist_musica.playlist_id) inner join musica on (playlist_musica.musica_id = musica.id) where usuario.id = 1 and playlist.id = 1;

select artista.id, artista.nome, count(musica.id) as qtde from artista inner join album on (artista.id = album.artista_id) inner join musica on (album.id = musica.album_id) group by artista.id, artista.nome having count(musica.id) >= 2 order by artista.id, artista.nome;
...
select artista.nome FROM artista UNION select usuario.nome from usuario;

SELECT usuario.id, usuario.nome, playlist.nome, count(playlist_musica.musica_id) from playlist left join playlist_musica on (playlist.id = playlist_musica.playlist_id) inner join usuario on (usuario.id = playlist.usuario_id) group by playlist.nome, usuario.id, usuario.nome;

-- *** with - Retorne a média da quantidade de músicas de todas as playlists
...
-- considerando no denominador somente a qtde de playlist que tenham pelo menos 1 musica atrelada
WITH tabela_aux AS (
     select playlist.id, count(*) as qtde_por_playlist from playlist inner join playlist_musica on (playlist.id = playlist_musica.playlist_id) group by playlist.id
) select avg(qtde_por_playlist) from tabela_aux;

-- usando como denominador todas as playslist - ate mesmo aquelas que nao tem musica atrelada
WITH tabela_aux AS (
     select playlist.id, count(*) as qtde_por_playlist from playlist inner join playlist_musica on (playlist.id = playlist_musica.playlist_id) group by playlist.id
) select sum(qtde_por_playlist)::real/(select count(*) from playlist)::real from tabela_aux;



-- Retorne somente playlists que possuem quantidade de músicas maior ou igual a média
WITH tabela_aux AS (
     select playlist.id, count(*) as qtde_por_playlist 
        from playlist inner join playlist_musica 
            on (playlist.id = playlist_musica.playlist_id) 
    group by playlist.id
)  select playlist.id, playlist.nome, count(*) 
    from playlist inner join playlist_musica 
on (playlist.id = playlist_musica.playlist_id) 
    group by playlist.id, playlist.nome 
    having count(*) >= (select avg(qtde_por_playlist) as media from tabela_aux);
